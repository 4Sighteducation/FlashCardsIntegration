...
function saveFlashcardUserData(userId, data, callback) {
  ...
  // Add topic lists if available - ADDED THIS
  if (data.topicLists) {
    updateData[FIELD_MAPPING.topicLists] = JSON.stringify(data.topicLists || []);
  }
  
  // Add topic metadata if available - ADDED THIS
  if (data.topicMetadata) {
    updateData[FIELD_MAPPING.topicMetadata] = JSON.stringify(data.topicMetadata || []);
  }
  ...
  debugLog("SAVING TO KNACK: PUT DATA", updateData);
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      debugLog("KNACK SAVE SUCCESS RESPONSE", response);
      callback(true);
    },
    error: function(error) {
